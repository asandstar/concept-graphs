## data_common.py
**定义和加载支持多种 RGB-D 数据集（如 NICE-SLAM、Replica、TUM RGB-D、Azure Kinect 等）的 PyTorch Dataset 类，供后续 SLAM 或 3D 重建系统使用**。它统一了不同数据集的读取逻辑，并且可以选择性加载颜色图、深度图、相机位姿、内参矩阵，以及特征嵌入（如 LSeg 或 CLIP 的语义特征）。
...
2. **支持图像和深度的下采样、归一化、channels-first等预处理**；
3. **支持不同格式的位姿文件（如 `.txt`, `.npy`, `.log`, `.jsonl` 等）**；
...
### 🎯 如果你希望换一个自己的 RGB-D 数据集，你需要做：

#### 🚦情况一：你已有的数据格式与现有 Dataset 类（如 `ReplicaDataset`, `ScannetDataset`, `RealsenseDataset` 等）相同或类似
...
> `config.yaml` 文件里要写明 `dataset_name`、相机内参等信息（示例如下）

---

#### 🚦情况二：你有一个全新的 RGB-D 数据集，结构和其他的不同
...
你可以将 `dataset_name` 改成 `realsense`、`azure`、`scannet`、`multiscan` 等现有的，或是你自定义类的名字。

---

### 💬 总结

- 本脚本核心是“**让不同数据集都能返回 color, depth, pose, K, embedding，统一格式进行处理**”；
- 换数据集主要就是：
  - 写清楚 `config.yaml`；
  - 数据放好，结构明确（比如 `rgb/*.png`, `depth/*.png`, `pose/*.txt`）；
  - 如果格式不同，就继承 `GradSLAMDataset`，自己写一个小类。

## conceptgraphs_datautils.py
提供了一些 **用于图像和相机内参预处理、位姿转换、标签图像生成** 的工具函数，是整个 `conceptgraph` 数据加载与处理流程中的基础工具之一。
...
### ✉️ 3. GPT Prompt 长这样（在 `GPTPrompt.py` 中）
